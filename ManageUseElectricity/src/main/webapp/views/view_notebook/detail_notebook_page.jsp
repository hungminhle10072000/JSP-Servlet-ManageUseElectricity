    ...
    <div class="sidebar-links" style="margin-top: 1rem">
        <ul>
            <li class="tooltip-element" data-tooltip="0">
                <a class="active"
                   href="${pageContext.request.contextPath}/NoteBookController" data-active="0">
                    <div class="icon">
...
<main>
    ...
    <form method="post" action="${pageContext.request.contextPath}/NoteBookController" accept-charset="utf-8">
        <input hidden name="idUpdate" value="${detailNoteBook.id}">
        <input hidden name="typePost" value="update">
        <input hidden name="selectElectric" value="${detailNoteBook.electricMeter.id}">
        <div>
            <label for="dateWrite">Date Write</label>
            <input style="display: block; height: 2rem" type="date" id="dateWrite" value=
            <fmt:formatDate pattern="yyyy-MM-dd" value="${detailNoteBook.dateWrite}"/>
        ...
        <div>
            ...
            <input required type="number" min="0" id="indexElectric" name="indexElectric"
                   value="${detailNoteBook.indexElectric}"
        ...
        <div style="margin-top: 1rem; margin-bottom: 1rem">
            ...
            <div style="width:200px;">
                <select disabled="disabled" name="selectElectric" id="selectElectric" required>
                    ...
                    <c:if test="${electricMeterList.size() > 0}">
                        <c:forEach items="${electricMeterList}" var="electric">
                            <option ${electric.id == detailNoteBook.electricMeter.id ? 'selected="selected"' : ''}
                                    value="${electric.id}">${electric.id}</option>
                        </c:forEach>
                    </c:if>
                </select>
            </div>
