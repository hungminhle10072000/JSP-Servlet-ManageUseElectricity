...
<head>
    ...
    <title>Detail Electric Meter</title>
    ...
    <div class="sidebar-links" style="margin-top: 1rem">
        <ul>
            <li class="tooltip-element" data-tooltip="0">
                <a class="active" href="${pageContext.request.contextPath}/ElectricMeterController" data-active="0">
                    <div class="icon">
...
<main>
    ...
    <form method="post" action="${pageContext.request.contextPath}/ElectricMeterController" accept-charset="utf-8">
        <input hidden name="idElectric" value="${electricDetail.getId()}"/>
        <input hidden name="typePostElectric" value="update">
        <div>
            ...
            <input style="display: block; width: 100%; height: 2rem" type="text" id="typeElectricMeter"
                   value="${electricDetail.typeElectricMeter}"
                   name="typeElectricMeter" required>
        </div>

        <div style="margin-top: 1rem; margin-bottom: 1rem">
            <label for="selectContract" style="margin-bottom: 2rem;">Select Customer</label>
            <div style="width:200px;">
                <select name="selectContract" id="selectContract" required>
                    <option value="0">Select Contract</option>
                    <c:if test="${contractList.size() >0}">
                        <c:forEach items="${contractList}" var="contract">
                            <option ${contract.id == electricDetail.contract.id ? 'selected="selected"' : ''}
                                    value="${contract.getId()}">${contract.getId()}</option>
                        </c:forEach>
                    </c:if>
                </select>
            </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#btnSubmit").click(function (event) {
            let checkConfirm = confirm("Are you sure you want to update electric meter !!!");
            if (checkConfirm) {
